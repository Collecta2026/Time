...
def _compute_run(run):
    ...
    # keep any comments already entered, keyed by employee
    prior = {p.employee_id: p.comment for p in
             Payslip.query.filter_by(run_id=run.id).all() if p.comment}
...
def payroll_detail(rid):
    ...
    dept_buckets = {}
    for p in slips:
        dn = p.employee.department.name if p.employee.department else "—"
        b = dept_buckets.setdefault(dn, {"name": dn, "headcount": 0, "net": 0.0,
                                         "employer_cost": 0.0})
        b["headcount"] += 1
        b["net"] += p.net_pay
        b["employer_cost"] += p.employer_cost
    dept_rows = [dept_buckets[d] for d in sorted(dept_buckets)]
    return render_template("payroll_run.html", run=run, slips=slips, totals=totals,
                           signoffs=signoffs, dept_rows=dept_rows)
...
@app.route("/payslip/<int:pid>/comment", methods=["POST"])
@require_role("hr", "finance_manager", "md")
def payslip_comment(pid):
    p = db.session.get(Payslip, pid) or abort(404)
    p.comment = (request.form.get("comment") or "").strip()[:300]
    audit("payslip.comment", p.run.period_label, f"{p.employee.name}: {p.comment}")
    db.session.commit()
    flash("Comment saved.", "ok")
    return redirect(url_for("payroll_detail", rid=p.run_id) + f"#e{p.employee_id}")
...
@login_required
def report_payroll():
    """Payroll summary by department with an organisation total, for one run."""
    runs = PayrollRun.query.order_by(PayrollRun.year.desc(), PayrollRun.month.desc()).all()
    rid = _int(request.args.get("run"))
    run = db.session.get(PayrollRun, rid) if rid else (runs[0] if runs else None)
    groups, org = [], None
    if run:
        slips = Payslip.query.filter_by(run_id=run.id).join(Employee).all()
        buckets = {}
        for p in slips:
            dep = p.employee.department.name if p.employee.department else "—"
            buckets.setdefault(dep, []).append(p)

        def summarise(name, ps):
            return {
                "name": name, "headcount": len(ps),
                "gross": sum(p.gross for p in ps),
                "si_employee": sum(p.si_employee for p in ps),
                "si_employer": sum(p.si_employer + p.emergency_fund for p in ps),
                "tax": sum(p.income_tax for p in ps),
                "deductions": sum(p.total_deductions for p in ps),
                "net": sum(p.net_pay for p in ps),
                "employer_cost": sum(p.employer_cost for p in ps),
            }
        groups = [summarise(d, buckets[d]) for d in sorted(buckets)]
        org = summarise("Organisation total", slips)
        if request.args.get("export"):
            headers = ["Department", "Headcount", "Gross", "Employee SI", "Employer SI",
                       "Income tax", "Total deductions", "Net pay", "Employer cost"]
            rows = [[g["name"], g["headcount"], g["gross"], g["si_employee"],
                     g["si_employer"], g["tax"], g["deductions"], g["net"],
                     g["employer_cost"]] for g in groups]
            rows.append([org["name"], org["headcount"], org["gross"], org["si_employee"],
                         org["si_employer"], org["tax"], org["deductions"], org["net"],
                         org["employer_cost"]])
            return _xlsx_response(f"payroll_by_department_{run.period_label}", headers, rows)
    return render_template("report_payroll.html", runs=runs, run=run,
                           groups=groups, org=org)
...
# --------------------------------------------------------------------------- #
def _ensure_schema():
    """Add any columns that newer versions introduced but an older live DB lacks.
    Safe and idempotent: only ever ADDs missing, nullable columns."""
    ...
    for table in db.metadata.sorted_tables:
        ...
        for col in table.columns:
            ...
            try:
                ...
                db.session.rollback()
                print(f"schema: could not add {table.name}.{col.name}: {exc}")
